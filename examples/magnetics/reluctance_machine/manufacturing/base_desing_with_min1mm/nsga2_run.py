...
if __name__ == '__main__':
    class MyProblem(ElementwiseProblem):
        def __init__(self):
            super().__init__(n_var=9,
                             ...
                             n_ieq_constr=0,
                             n_eq_constr=0,
                             xl=np.array([15, 10, 1, 1, 2, 10, 10, 0, 0]),
                             xu=np.array([25, 15, 4, 4, 2, 15, 18, 8, 8]),
                             vtype=int)

        def _evaluate(self, x, out, *args, **kwargs):

            f1 = calc_torque_avg_rip.torque_avg_rip(30, x[0], x[1], x[2], 1, x[3], x[4], 1.5, x[5], x[6], x[7], x[8])
    ...
    file_path = os.path.join(folder_path, f'results/all_res_cog_case6_20250306.csv')

    termination = get_termination("n_size", str(file_path), 20)
    ...
    file_path = os.path.join(folder_path, f'results/nsga2_case6_p100o100g200_var9_20250306.csv')
