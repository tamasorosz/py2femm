...
def torque_avg_rip(I0, ang_co, deg_co, bd, bw, bh, bgp, mh, ang_m, ang_mp, deg_m, deg_mp):
    ...
    file_path = folder_path / "results" / "all_res_avg_case6_20250306.csv"
    ...
    if not file_path.exists() or file_path.stat().st_size == 0:
        if ang_m != ang_mp:
            if not (deg_m == 0 and deg_mp == 0):
                ...
                print(
                    f'{int(ang_co), int(deg_co), int(bd), int(bw), int(bh), int(bgp), int(mh), int(ang_m), int(ang_mp), int(deg_m), int(deg_mp)}' +
