...
for C in [5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 75]:
    ...
    tag5A = pd.read_csv(current_dir + '/results/' + f'taguchi_{C}A.csv')
    ...
    for i in range(x):
        for j in range(y):
            SNavg_temp1 = [tag5A.iloc[L25.iloc[j, i][k], 7] for k in range(len(L25.iloc[j, i]))]
    ...
    df = pd.DataFrame(result_array, columns=['X' + str(i) for i in range(1, 7)])

    difference = df.max() - df.min()
    new_row_df = pd.DataFrame([difference.values], columns=difference.index)
    df = pd.concat([df, new_row_df], ignore_index=True)
    df = df.rename(index={0: 'L1', 1: 'L2', 2: 'L3', 3: 'R'})

    current_dir = os.getcwd()
    file_path = current_dir + '/results/' + f'res_avg_taguchi_{C}A.csv'
