...
def process_simulation(i):
    try:
        ...
        femm.opendocument(f'fem_20/cog_{i}.fem')
...
if __name__ == '__main__':
    ...
    femm.openfemm()
    femm.opendocument("steel_full_exc000_gl000_ferrite_x10_mesh020n66676.fem")

    os.mkdir("fem")
    for i in values:
        femm.mi_saveas(f'fem_20/cog_{i}.fem')
    femm.closefemm()

    # Check CPU count to determine number of workers
    num_processes = 6  # Leave one core for OS
    print(f"Starting simulation with {num_processes} processes...")

    # Clear the file initially (optional, or use 'a' to keep appending)
    with open("2D_FEMM_Steel_18mm_full_exc000_gl000_mag10_mesh020.csv", "w") as f:
        pass
    ...
    with open("2D_FEMM_Steel_18mm_full_exc000_gl000_mag10_mesh020.csv", "a") as file_out:
        for res in results:
            if res is not None:
