...
def process_simulation(i):
    try:
        ...
        femm.opendocument(f'fem_25/cog_{i}.fem')
...
if __name__ == '__main__':
    ...
    femm.openfemm()
    femm.opendocument("steel_full_exc000_gl000_ferrite_x10_mesh025n55880.fem")

    os.mkdir("fem_25")
    for i in values:
        femm.mi_saveas(f'fem_25/cog_{i}.fem')
    femm.closefemm()

    # Check CPU count to determine number of workers
    num_processes = 4  # Leave one core for OS
    print(f"Starting simulation with {num_processes} processes...")

    # Clear the file initially (optional, or use 'a' to keep appending)
    with open("2D_FEMM_Steel_18mm_full_exc000_gl000_mag10_mesh025.csv", "w") as f:
        pass
    ...
    with open("2D_FEMM_Steel_18mm_full_exc000_gl000_mag10_mesh025.csv", "a") as file_out:
        for res in results:
            if res is not None:
