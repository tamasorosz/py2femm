...
def torque_avg_rip(I0, ang_co, deg_co, bd, bw, bh, bg):
    initial = calc_max_torque_angle.max_torque_angle(I0, ang_co, deg_co, bd, bw, bh, bg)
    ...
    for counter, ia, alpha in zip(range(0, resol), np.linspace(0, e, resol), np.linspace(0, 4 * e, resol)):
        variables = model.VariableParameters(fold='avg_rip',
                                             ...
                                             IBp=I0 * math.cos(math.radians(initial + alpha + 120)),
                                             IBn=-I0 * math.cos(math.radians(initial + alpha + 120)),
                                             ICp=I0 * math.cos(math.radians(initial + alpha + 240)),
                                             ICn=-I0 * math.cos(math.radians(initial + alpha + 240)),
                                             ang_co=ang_co,
                                             deg_co=deg_co * 10,
                                             bd=bd,
                                             bw=bw,
    ...
    if None in res:
        ...
        torque_avg = np.round(-1 * np.average(res), 3)
        torque_ripple = np.round(-100 * (np.max(res) - np.min(res)) / torque_avg, 3)

    # res.clear()  # To make sure that there is no memory leak

    torque_angle = -1 * initial

    print(f'{float(ang_co), float(deg_co), float(bd), float(bw), float(bh), float(bg)}' +
          '\n' + 'ANG: ' + f'{initial}' + ', AVG: ' + f'{-1 * torque_avg}' + ', RIP: ' + f'{torque_ripple}' +
          '\n-----------------------------------------------')

    df = pd.DataFrame({
        'X1': [ang_co], 'X2': [np.round(deg_co * 10, 2)], 'X3': [bd], 'X4': [bw],
        'X5': [bh], 'X6': [np.round(bg * 0.5, 2)], 'ANG': [torque_angle], 'AVG': [torque_avg], 'RIP': [torque_ripple]})

    current_file_path = os.path.abspath(__file__)
    folder_path = os.path.dirname(current_file_path)
    file_path = os.path.join(folder_path, f'results/all_res_avg_case0_20250305.csv')
