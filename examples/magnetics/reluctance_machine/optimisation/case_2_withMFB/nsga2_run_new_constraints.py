...
if __name__ == '__main__':
    class MyProblem(ElementwiseProblem):
        def __init__(self):
            super().__init__(n_var=9,
                             ...
                             n_ieq_constr=0,
                             n_eq_constr=0,
                             xl=np.array([15,  6,  .5, .001, .5, 1, 1.5, 10, 10]),
                             xu=np.array([25, 14,   4,    1,  4, 2,   2, 15, 18]),
                             vtype=float)
    ...
    df = pd.DataFrame({
        ...
        'X8': X[:, 7], 'AVG': F[:, 0], 'RIP': F[:, 1], 'COG': F[:, 2]})
    ...
    file_path = os.path.join(folder_path, f'results/nsga2_case2_20251105.csv')
