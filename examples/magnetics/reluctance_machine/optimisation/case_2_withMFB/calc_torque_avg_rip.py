...
def torque_avg_rip(I0, ang_co, deg_co, bd, bw, bh, bgp, mh, ang_m, ang_mp):
    if mh > 1.5:
        ...
        print(f'ang_co: {ang_co}|', f'deg_co: {deg_co}|', f'bd: {bd}|', f'bw: {bw}|', f'bh: {bh}|', f'bgp: {bgp}|',
              f'mh: {mh}|', f'ang_m: {ang_m}|', f'ang_mp: {ang_mp}|',
              '\n' + 'ANG: ' + f'{initial}' + ', AVG: ' + f'{-1 * torque_avg}' + ', RIP: ' + f'{torque_ripple}')

        df = pd.DataFrame({
            'X1': [ang_co], 'X2': [np.round(deg_co * 10, 2)], 'X3': [bd], 'X4': [bw],
            'X5': [bh], 'X6': [np.round(bgp * 0.5 + mh, 2)], 'X7': [mh], 'X8': [ang_m], 'X9': [ang_mp],
            'ANG': [torque_angle], 'AVG': [torque_avg], 'RIP': [torque_ripple]
        })

        current_file_path = os.path.abspath(__file__)
        folder_path = os.path.dirname(current_file_path)
        file_path = os.path.join(folder_path, f'results/all_res_avg_case2_20250429_all_variable.csv')
