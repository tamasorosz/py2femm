...
def torque_avg_rip(I0, ang_co, deg_co, bd, bw, bh, bgp, mh, ang_m, deg_m):
    if mh >= 1.5:
        initial = calc_max_torque_angle.max_torque_angle(I0, ang_co, deg_co, bd, bw, bh, bgp, mh, ang_m, deg_m)
        ...
        print(f'ang_co: {ang_co}|', f'deg_co: {deg_co}|', f'bd: {bd}|', f'bw: {bw}|', f'bh: {bh}|', f'bgp: {bgp}|',
              f'mh: {mh}|', f'ang_m: {ang_m}|', f'deg_m: {deg_m}|',
        ...
        df = pd.DataFrame({'X1': [ang_co], 'X2': [np.round(deg_co * 10, 2)], 'X3': [bd], 'X4': [bw],
                           ...
                           'X9': [deg_m], 'ANG': [torque_angle], 'AVG': [torque_avg], 'RIP': [torque_ripple]})

        current_file_path = os.path.abspath(__file__)
        folder_path = os.path.dirname(current_file_path)
        file_path = os.path.join(folder_path, f'results/all_res_avg_case3_20250421_all_variable.csv')
