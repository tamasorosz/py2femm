...
#
# ax = plt.gca()
# ellipse = Ellipse((61.5, 1783), 1, 30, edgecolor=colors[2], facecolor=colors[2], linewidth=3)
# ax.add_patch(ellipse)
#
# ax = plt.gca()
...
#
# plt.annotate('Variance weighted', xy=(61.5, 1783), xytext=(55, 1400),
#              arrowprops=dict(facecolor=colors[3], shrink=0.05),
#              fontsize=20, color='black', ha='center')
#
...
ax.axhline(y=np.mean(f.iloc[:, 0]), color=colors[0], linestyle='--', linewidth=2,
               label='avg(V): ' + f'{int(np.mean(f.iloc[:, 0]))}' + ' mNm')
ax.axhline(y=np.mean(f.iloc[:, 1]), color=colors[1], linestyle='--', linewidth=2,
               label='avg(E): ' + f'{int(np.mean(f.iloc[:, 1]))}' + ' mNm')
...
ax.plot([], color=colors[0], linestyle='None', label='rip(V): ' + f'{ripV}' + ' %')
ax.plot([], color=colors[1], linestyle='None', label='rip(E): ' + f'{ripE}' + ' %')
