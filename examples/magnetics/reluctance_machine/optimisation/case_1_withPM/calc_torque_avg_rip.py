...
def torque_avg_rip(I0, ang_co, deg_co, bd, bw, bh, bgp, mh, ang_m):
    ...
    print(f'{ang_co, deg_co, bd, bw, bh, bgp, mh, ang_m}' +
          '\n' + 'ANG: ' + f'{initial}' + ', AVG: ' + f'{-1 * torque_avg}' + ', RIP: ' + f'{torque_ripple}')

    df = pd.DataFrame({
        'X1': [ang_co], 'X2': [deg_co * 10], 'X3': [bd], 'X4': [bw],
        'X5': [bh], 'X6': [bgp * 0.5 + mh], 'X7': [mh], 'X8': [ang_m],
        'ANG': [torque_angle], 'AVG': [torque_avg], 'RIP': [torque_ripple]})

    current_file_path = os.path.abspath(__file__)
    folder_path = os.path.dirname(current_file_path)
    file_path = os.path.join(folder_path, f'results/all_res_avg_case1_20250223.csv')
