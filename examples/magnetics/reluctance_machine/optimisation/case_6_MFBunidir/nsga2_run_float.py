...
if __name__ == '__main__':
    class MyProblem(ElementwiseProblem):
        def __init__(self):
            super().__init__(n_var=9,
                             ...
                             xu=np.array([25, 15, 4, 4, 2, 15, 18, 16, 16]),
                             vtype=float)

        def _evaluate(self, x, out, *args, **kwargs):

            f1 = calc_torque_avg_rip.torque_avg_rip(30, x[0], x[1], x[2], 0, x[3], x[4], 1.5, x[5], x[6], x[7], x[8])
            f2 = calc_cogging.cogging(0, x[0], x[1], x[2], 0, x[3], x[4], 1.5, x[5], x[6], x[7], x[8])
    ...
    class MyRepair(Repair):
        ...
        def _do(self, problem, x, **kwargs):
            ...
            for i in range(len(x)):
                ...
                if g > 0:
                    temp_x3 = 8 - (math.tan(math.radians(x[i][0] / 2)) * (22 - (x[i][4] * 0.5 + 1.5))) - x[i][2]
                    if temp_x3 < 1:
                        x[i][3] = 1
    ...
    file_path = os.path.join(folder_path, f'results/all_res_cog_case6_20250327_no_rib.csv')
    ...
    file_path = os.path.join(folder_path, f'results/nsga2_case6_p100o100g200_var9_20250327_no_rib.csv')
