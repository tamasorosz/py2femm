...
# AVERAGE TORQUE%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
feature_names = ['C1', 'C2', 'C3', 'C4', 'C5', 'C6', 'C7', 'C8', 'C9']
f_scores = f_avg
p_values = p_avg
...
plt.tight_layout()
plt.savefig('sensitivity_case3_avg.png', dpi=300)
plt.show()
...
plt.gca().invert_yaxis()  # Highest score at top
plt.tight_layout()
plt.savefig('sensitivity_case3_rip.png', dpi=300)
plt.show()
...
plt.xlabel('F-score', fontsize=18)
plt.ylabel('Features', fontsize=18)
plt.xticks(ticks=np.arange(0,61,10), fontsize=18)
plt.yticks(fontsize=18)
plt.grid(True)
plt.gca().set_axisbelow(True)
...
for i, (bar, pval) in enumerate(zip(bars, p_values)):
    plt.text(
        bar.get_width() + 1.5,              # X position (just beyond bar)
        bar.get_y() + bar.get_height() / 2, # Y position (center of bar)
        f'p={pval:.3g}',                    # Format p-value
        va='center', fontsize=18, color='black',
        bbox = dict(facecolor='white', alpha=1, edgecolor='none', boxstyle='round,pad=0.2')
...
plt.gca().invert_yaxis()  # Highest score at top
plt.tight_layout()
plt.savefig('sensitivity_case3_cog.png', dpi=300)
plt.show()
