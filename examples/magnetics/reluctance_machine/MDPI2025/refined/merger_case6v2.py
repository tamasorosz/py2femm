...
with open('../raw/all_res_avg_case6_20250904_10X.csv', 'r') as f:
    df1 = pd.read_csv(f)

with open('../raw/all_res_cog_case6_20250904_10X.csv', 'r') as f:
    df2 = pd.read_csv(f)

# Get the first 9 columns from both DataFrames
df1_subset = df1.iloc[:, :10]
df2_subset = df2.iloc[:, :10]
print(len(df1_subset))
print(len(df2_subset))
...
# merged_x.to_csv('case6_cogging.csv', index=False)
...
# merged_x.to_csv('case6_average.csv', index=False)

dfx_subset = merged_x.iloc[:, :1]
dfy_subset = merged_y.iloc[:, :1]
