...
cog2 = df3['COG'].values[np.newaxis, :]  # shape (1, len(df2))
...
cog_diff_df = pd.DataFrame(cog_diff, index=df1.index, columns=df3.index)

cog_diff_df.to_parquet("diff_cog_df_case1_case3.parquet", index=False)