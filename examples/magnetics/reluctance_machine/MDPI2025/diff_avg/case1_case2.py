...
avg2 = df2['AVG'].values[np.newaxis, :]  # shape (1, len(df2))
...
avg_diff_df = pd.DataFrame(avg_diff, index=df1.index, columns=df2.index)

avg_diff_df.to_parquet("diff_avg_df_case1_case2.parquet", index=False)