...
rip2 = df3['RIP'].values[np.newaxis, :]  # shape (1, len(df2))
...
rip_diff_df = pd.DataFrame(rip_diff, index=df1.index, columns=df3.index)

rip_diff_df.to_parquet("diff_rip_df_case1_case3.parquet", index=False)